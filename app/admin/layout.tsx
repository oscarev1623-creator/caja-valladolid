...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // ✅ Detectar si es una página de chat individual (no la lista)
  const isChatDetailPage = pathname?.startsWith('/admin/chat/') && pathname !== '/admin/chat'
  ...
  // ✅ En móvil y chat individual, ocultar sidebar y header automáticamente
  const hideSidebar = isMobile && isChatDetailPage
  ...
  const navItems: NavItem[] = [
    ...
    { name: 'Asesores', href: '/admin/agents' },
    { name: 'Administradores', href: '/admin/users' },
  ]

  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Sidebar - OCULTAR en móvil cuando es chat individual */}
      {!hideSidebar && (
      ...
      {/* Contenido principal - SIN MARGEN en móvil/chat */}
      <div className={`flex-1 transition-all ${
        hideSidebar ? 'ml-0' : (sidebarOpen ? 'ml-64' : 'ml-20')
      }`}>
        {/* Top bar - OCULTAR en móvil cuando es chat individual */}
        {!hideSidebar && (
        ...
        {/* Contenido - SIN PADDING en móvil/chat */}
        <main className={hideSidebar ? 'p-0' : 'p-6'}>
          {children}
        </main>
      </div>
    </div>
