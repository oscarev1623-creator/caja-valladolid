...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!leadId) {
      return NextResponse.json({ success: false, error: 'Lead ID requerido' }, { status: 400 })
    }

    const lead = await prisma.lead.findUnique({ where: { id: leadId } })

    if (!lead || !lead.email) {
      return NextResponse.json({ success: false, error: 'Lead no encontrado o sin email' }, { status: 404 })
    }

    console.log('📄 Generando PDF para enviar por correo:', lead.id)
    ...
    clausulas.forEach(c => { doc.text(c, 20, y); y += 5 })
    y += 5
    ...
    pasos.forEach(p => { doc.text(p, 20, y); y += 6 })
    ...
    clausulaSexta.forEach(c => { doc.text(c, 20, y); y += 5 })
    y += 15
    ...
    const filename = `CONTRATO_${nombreCliente.replace(/\s/g, '_')}.pdf`

    // Enviar correo con el contrato
    ...
    console.log('✅ Contrato enviado por correo a:', lead.email)
